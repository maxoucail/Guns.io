/**
 * link2me — GitHub Storage Service
 * Uploads images/videos to GitHub for CDN delivery
 */
'use strict';
...
class GitHubStorageService {
  ...
  async _ensureBranch() {
    ...
    try {
      ...
      console.log(`[GitHubStorage] Branche '${this.branch}' créée`);
      this._branchReady = true;
    }
  }
  ...
  async _getRef() {
    const { data } = await this.client.get(`/repos/${this.owner}/${this.repo}/git/ref/heads/${this.branch}`);
    return data;
  }

  async _getCommit(sha) {
    const { data } = await this.client.get(`/repos/${this.owner}/${this.repo}/git/commits/${sha}`);
    return data;
  }
  ...
  async _createTree(baseTree, filePath, blobSha) {
    const { data } = await this.client.post(
      ...
      { base_tree: baseTree, tree: [{ path: filePath, mode: '100644', type: 'blob', sha: blobSha }] }
    );
    return data;
  }
